...
### Added
- **Token provider for refresh without environment variables**: `set_token_provider(callable)` registers a zero-argument function that returns a fresh bearer token. On a `401`, `_request_with_retry` now prefers the provider, falls back to `authenticate()` only when a complete set of `AQUA_*` variables is present (`auth.env_credentials_present()`), and otherwise returns the `401` to the caller. Applications that hold their own credentials and sign in with `api_auth()` no longer get dragged into an environment-based re-auth they never configured. `get_token_provider()` and `clear_token_cache()` are exported alongside it
- **`aquasec.exceptions`**: `AquaError` base class (`.status_code`, `.response_text`, None unless the error came from an HTTP response) with `AuthenticationError`, `MissingCredentialsError` and `ApiError`. All exported from the package root. **Every** error the library raises on purpose is now an `AquaError`: the ~25 `raise Exception(...)` sites across `code_repositories`, `containers`, `enforcers`, `exports`, `host_images`, `inventory`, `repositories`, `scopes`, `vms` and `vulnerabilities` raise `ApiError` (still an `Exception`, so existing `except Exception` handlers are unaffected)

### Changed
- **The library no longer calls `sys.exit()`.** `authenticate()` raises `MissingCredentialsError` (message unchanged, with a pointer to `set_token_provider()`); `api_auth()`, `user_pass_saas_auth()` and `user_pass_onprem_auth()` raise `AuthenticationError`; `get_enforcers_from_group()` and `get_enforcer_groups()` raise `ApiError`. A library that exits the host process on a transient `401` is not usable from a long-running service. Every example CLI already wrapped `authenticate()` in `except Exception` and exits 1 itself, so their behaviour is unchanged
- **Refreshed tokens are scoped to the token they replaced.** The old module-global cached token overrode whatever token a caller passed in once any re-auth had happened; now a refresh obtained for token A is applied only to later calls made with token A. The refresh is single-flight (concurrent 401s across threads sign in once), a token refreshed within the last `REFRESH_BACKOFF_SECONDS` (60) that is *still* rejected is returned as a 401 rather than triggering another sign-in on every request (a persistent 401 is a wrong host, role or scope, not an expiry), and the per-token maps are pruned of expired tokens and capped so a long-running process does not grow them forever

### Fixed
- **Supply Chain calls went to the wrong region without `AQUA_ENDPOINT`.** The Supply Chain API is served per region (`api.eu-1.supply-chain...`); when the console hostname carries no region (most tenants: `xxxx.cloud.aquasec.com`) the only place the region is known is the endpoint the token was issued from, and `_get_supply_chain_url()` read that from the `AQUA_ENDPOINT` environment variable alone. An application signing in with `api_auth()` and no environment variables was therefore sent to the region-less US host and got `401 Unauthorized` for a perfectly valid token. Reproduced live on an eu-1 tenant: same `ANY:*` token, `api.eu-1...` → 200, `api.supply-chain...` → 401. `api_auth()` and `user_pass_saas_auth()` now record the endpoint they signed in against **per token** (`auth.get_api_endpoint(token)`, `auth.set_api_endpoint(endpoint, token)`), and both the Supply Chain URL and the export-service region are derived from it. Resolution order: this token's record → `AQUA_ENDPOINT` → the most recent sign-in in the process → none (US host); so a process holding tokens for two tenants is not misled by whichever signed in last
- `authenticate_with()` (used by the setup wizard) swallows failures with `except Exception` and returns `None` — but `SystemExit` is not an `Exception`, so a rejected credential during setup killed the wizard instead of reporting. Follows from the change above
